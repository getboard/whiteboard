...
def on_add_text(ctx: context.Context, event: tkinter.Event, **kwargs):
    ...
    obj_id = ctx.objects_storage.create(
        'TEXT', x=actual_x, y=actual_y, text='новый текст по клику', **kwargs
    )
...
def on_update_text(ctx: context.Context, event: tkinter.Event):
    ...
    if text.master is not None and text.widgetName == 'text':
        ...
        obj.update(ctx, text=text.get('1.0', 'end-1c'), width=width)
        ctx.events_history.add_event('EDIT_TEXT', obj_id=name, new_text=text.get('1.0', 'end-1c'))


def on_double_click(ctx: context.Context, event: tkinter.Event):
    ...
    obj: object_types.TextObject = ctx.objects_storage.get_by_id(obj_id)
    obj.show_text(txt, x=event.x, y=event.y, width=width, height=height, anchor='center')
