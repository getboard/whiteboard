...
def bind_generators(ctx: context.Context):
    ...
    ctx.canvas.bind(
        '<Double-1>', lambda event: generators.set_cursor(ctx, event), add="+")
    ctx.canvas.bind(
        '<Button-1>', lambda event: generators.set_focus(ctx, event), add="+")
    ...
    # ctx.canvas.bind("<FocusOut>", lambda event: generators.focus_out(ctx, event), add="+")
